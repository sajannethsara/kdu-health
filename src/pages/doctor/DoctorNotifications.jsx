...
export default function DoctorNotifications({doctorId}) {
      ...
      {notifications.length === 0 ? (
        ...
        <div className="space-y-3">
          {notifications.map((notification) => (
            <div 
              key={notification.id} 
              className="p-4 border mx-3 border-gray-200 rounded-xl bg-white hover:bg-gray-50 transition-colors shadow-sm"
            >
              <div className="flex gap-3">
                <Avatar className="h-11 w-11 border-2 border-gray-100 flex-shrink-0">
                  <AvatarImage 
                    src={notification.studentAvatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(notification.studentName || 'Student')}`}
                    alt="Student Avatar"
                  />
